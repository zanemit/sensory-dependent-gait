...
for i, (limb_str, clr, added) in enumerate(zip(
    ...
    sample_size = 1000 if da_3d.coords['exp'].shape[0]>1000 else da_3d.coords['exp'].shape[0]
    for trace in np.random.choice(da_3d.coords['exp'].values, sample_size, replace=False):
        ax.plot(da_3d.loc[:, trace, limb_str]+added, color=FigConfig.colour_config[clr][clr_id], alpha=0.03)
