...
import scipy.stats
from processing import data_loader, utils_processing, treadmill_linearGLM
from processing.data_config import Config
from figures.fig_config import Config as FigConfig

def plot_fig4G(yyyymmdd, outcome_variable, ref, rH_category,
            set_sba_ant=40, set_sba_post=40):
    appdx = f"_incline_rH{rH_category}"
    catvar = None
    
    yticks = np.linspace(-0.1,0.2,4)
    ylims=(-0.13, 0.2)
    ytexts = ('diagonal', 'hindlimb\n1-limb')
    ...
    datafull = data_loader.load_processed_data(outputDir = Config.paths['passiveOpto_output_folder'],
                                                ...
                                                limb = ref, 
                                                appdx = appdx)[0]
    datafull['incline'] = datafull['headLVL'].apply(lambda s: -int(s[3:]))
    ...
    #-------------PLOT!!------------------- 
    fig, ax = plt.subplots(1,1, figsize = (1.25,1.45)) 
    ...
    if os.path.exists(mod_path):
        ...
        p2 = stats.loc[f'pred2_centred:pred{np.argwhere(np.asarray(predictorlist)==predictor)[0][0]+1}_centred', "Pr(>|t|)"]
        for i, p in enumerate([p1, p2]):
            ...
            if i==1:
                ax.text(set_sba_ant + (0.5*(set_sba_post-set_sba_ant)),
                        ylims[1]-((ylims[1]-ylims[0])*0.16),
                        f"angle x {predictor_str.split(' ')[-1]}: {ptext}",
                        ha='center',
                        fontsize=5)
            else:
                ax.text(set_sba_ant + (0.5*(set_sba_post-set_sba_ant)),
    ...
    figtitle = f"passiveOpto_{outcome_variable}_vs_incline.svg"
...
if __name__=="__main__":
    ...
    yyyymmdd = '2022-08-18'
    rH_category ="asym" 
    plot_fig4G(yyyymmdd, outcome_variable, ref, rH_category)