...
for reflimb_id, (clr, lnst, lbl) in enumerate(zip(
    ...
    # add texts
    ax.text(set_sba_ant+2+(17*reflimb_id), 
            ylims[1]-((ylims[1]-ylims[0])*0.16),
            lbl,
            fontsize=5,
            color=clr)
    ax.hlines(ylims[1]-((ylims[1]-ylims[0])*0.17),
              set_sba_ant+2+(17*reflimb_id),
              set_sba_ant+12+(17*reflimb_id),
            ls=lnst,
            lw=0.7,
            color=clr)
...
if os.path.exists(mod_path):
    ...
    for i, p in enumerate([p1, p2]):
        ...
        if i==1:
            ax.text(set_sba_ant + (0.3*(set_sba_post-set_sba_ant)),
                    ylims[1]-((ylims[1]-ylims[0])*0.16),
                    f"vs          trials: {ptext}",
                    fontsize=5)
        else:
