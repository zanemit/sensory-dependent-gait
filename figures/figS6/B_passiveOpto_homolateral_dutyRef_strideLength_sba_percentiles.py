...
for iprcnt, (prcnt, speed, lnst) in enumerate(zip(prcnts,
    ...
    for k, (lo, hi) in enumerate(zip([-np.pi, 0, np.pi] , [np.pi, 2*np.pi, 3*np.pi])):
        ...
        if k == 2:
            ...
            ax.hlines(ylim[1]-0.7, 151+7*iprcnt, 156+7*iprcnt, color = c, ls = lnst, lw = 1)
            ax.text(xlim[0] + (0.3 * (xlim[1]-xlim[0])) + 7*iprcnt,
...
ax.text(xlim[0] + (0.1 * (xlim[1]-xlim[0])),
        ...
        f"stride length x angle: {stat_dict['pred2:pred3']}",
        fontsize=5)
