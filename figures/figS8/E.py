...
def plot_figS8E(dep_var, indep_var, limbRef, group_num=5):
    fig, ax = plt.subplots(1, 1, figsize=(1.35, 1.4))
    ...
    n_data = []
    ssrs = []
    for i, (yyyymmdd, otp_dir, appdx, data_str, indep_col, clr, lnst, lbl) in enumerate(zip(
                                    ['2022-05-06', '2022-08-18'], 
                                    [Config.paths["mtTreadmill_output_folder"], Config.paths["passiveOpto_output_folder"]],
                                    ['', '_incline'],
        ...
        df, _, _ = data_loader.load_processed_data(outputDir = otp_dir, 
        ...
        if indep_var == 'incline':
            df = pd.DataFrame({'incline': [-int(x[3:]) for x in df[indep_col]],
                               'snoutBodyAngle': df['snoutBodyAngle'],
                               'mouseID': df['mouseID']})
        ...
        ax.set_xlim(-50,50)
        ax.set_ylim(135,180)
        ax.set_ylabel('Snout-body angle\n(deg)')
        ...
        ax.hlines(177.5, 1 - (i*40), 35 - (i*48), color = clr, ls = lnst, lw = 1)
    ...
    ax.text(-48,179, "passive ", ha = 'left', 
            color = FigConfig.colour_config['homolateral'][2], 
            fontsize = 5)
    p_text = "vs motorised:\n" + ('*' * (p_value < FigConfig.p_thresholds).sum())
    if (p_value < FigConfig.p_thresholds).sum() == 0:
        p_text += "n.s."
    ax.text(-15,179, p_text, ha = 'left', 
            color = FigConfig.colour_config['headbars'], 
            fontsize = 5)
    
    fig.savefig(Path(FigConfig.paths['savefig_folder']) / f"motorised_passive_snoutBodyAngles_vs_slope_COMPARISON.svg",
                dpi=300,
                transparent=True)
...
if __name__=="__main__":    
    ...
    limbRef='lH1'
    indep_var = 'incline'
    plot_figS8E(dep_var, indep_var, limbRef)
