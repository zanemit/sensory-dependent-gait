...
for ref_id, (lnst, clr, lbl) in enumerate(zip(['solid', 'dashed'],
                                            ['homolateral', 'greys'],
                                            ['L-hind', 'R-hind'])):
    c = FigConfig.colour_config[clr][2-ref_id]
    ...
    for k, (lo, hi) in enumerate(zip([-np.pi, 0, np.pi] , [np.pi, 2*np.pi, 3*np.pi])):
        ...
        if k == 2:
            ...
            ax.hlines(ylim[1]-0.3, -38+36.5*ref_id, -16+37*ref_id, color = c, ls = lnst, lw = 1)
            ax.text(xlim[0] + (0.04 * (xlim[1]-xlim[0])) + 36.5*ref_id,
...
cat_coef_str = f"pred{len(predictorlist)+1}rH1"
