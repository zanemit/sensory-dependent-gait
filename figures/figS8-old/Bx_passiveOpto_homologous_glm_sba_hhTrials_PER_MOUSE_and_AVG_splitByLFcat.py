...
# mouselist= np.asarray(['FAA1034836', 'FAA1034839', 'FAA1034842',  'FAA1034868', 
#                        'FAA1034869', 'FAA1034945', 'FAA1034949'])
...
#---------------PER MOUSE--------------------
for ref_id, (main_clr, LFlbl) in enumerate(zip(
        [FigConfig.colour_config['homologous'][1], FigConfig.colour_config['greys'][1]],
    ...
    for i in range(1, len(mouselist)+1):
        ...
        pp = phase_preds[:, :, predictor_id, i, ref_id]
        xr = x_range[:, predictor_id, i, ref_id]
        print(i, xr[-1]-xr[0])
    ...
    unique_traces = np.empty((0))           
    pp_avg = phase_preds_avg[:, :, predictor_id, 0, ref_id]            
    for k, (lo, hi) in enumerate(zip([-np.pi, 0, np.pi] , [np.pi, 2*np.pi, 3*np.pi])):
        print(f"Working on data range {k}...")
        if k == 1:
    ...
    figtitle = f"passiveOpto_{yyyymmdd}_{appdx}_{limb}_ref{ref}_{'_'.join(predictorlist)}_SLOPE{''.join(slopes)}_{interaction}_{appdx}_PER_MOUSE_{LFlbl}.svg"
