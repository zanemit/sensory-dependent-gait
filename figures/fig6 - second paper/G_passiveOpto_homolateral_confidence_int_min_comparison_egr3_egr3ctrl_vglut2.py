...
average_confints = {}
for i, (yyyymmdd, yyyymmdd_fp, appdx, sample_num, data_frac, mouselist, rfl_str) in enumerate(zip(
    ...
    ## COMPUTE THE CoMy-EQUIVALENT SBA RANGE
    ...
    get_CoMy_ant = CoMy_pred_intercept + (CoMy_pred_slope*(sba_min-np.nanmean(datafull[predictor]))) + np.nanmean(data_fp["CoMy_mean"])
    get_CoMy_post = CoMy_pred_intercept + (CoMy_pred_slope*(sba_max-np.nanmean(datafull[predictor]))) + np.nanmean(data_fp["CoMy_mean"])
    
    CoMy_target_range = np.linspace(
        get_CoMy_ant,
        get_CoMy_post,
        sba_index.shape[0],
        endpoint = True)*Config.forceplate_config["fore_hind_post_cm"]/2
    ...
    for i,m in enumerate(mouselist):
        ...
        sba_min_m = np.nanmin(datafull_m[predictor])
        sba_max_m = np.nanmax(datafull_m[predictor])
...
# ax.set_yticklabels(["-π", "-0.5π", "0", "0.5π"])
...
ax.hlines(0.66, 1.1, 1.9, color = 'black', linewidth = 0.25)
ax.hlines(0.66, 2.1, 3, color = 'black', linewidth = 0.25)
ax.hlines(0.86, 1.1, 3, color = 'black', linewidth = 0.25)
### -------------------------STATS-------------------------------------

ax.set_ylabel("AP CoS of minimum\nconfidence interval (cm)")    
plt.tight_layout()

figtitle = f"passiveOpto_{limb}_confidence_int_min_comparison_egr3_egr3ctrl_vglut2.svg"
