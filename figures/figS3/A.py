...
fig, ax = plt.subplots(1,2,figsize = (1.7*2,1.7), sharey = True) # (1.7*2,1.7) for 4 columns
for axid, appdx, param, clrs, ploc in zip(
    ...
    ax[axid].set_xticklabels([f'({keys[j].left:.0f},{keys[j].right:.0f}]' for j in range(group_num)], 
                             rotation = 35,
                             ha = 'right')

ax[0].set_ylabel("Circular optimal transport")
...
figtitle = f"{yyyymmdd}_idealised_comparison.svg"
