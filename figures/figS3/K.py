...
for i, (yyyymmdd, outputDir, lnst, x, appdx, marker) in enumerate(zip(
        ['2022-08-18', '2022-05-06'],
        [Config.paths["passiveOpto_output_folder"], Config.paths["mtTreadmill_output_folder"]],
        ['solid', 'dashed'],
        [np.arange(xmin, xmax+interval, interval), [-40,-20,0,20,40]],
        ['_incline', ''],
        ['p','m']
        )):
    ...
    if param == 'incline':
        df_reduced['incline'] = [-int(x[3:]) for x in df_reduced['headLVL']]
    ...
    ax.set_xlabel(xlbl)
    ax.text(-72, 0.15, "Hindlimb duty cycle",color = FigConfig.colour_config['hindlimbs'], rotation = 90)
