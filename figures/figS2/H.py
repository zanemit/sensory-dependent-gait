...
predictors = ['speed', 'snoutBodyAngle', 'weight']
param_col = 'speed'
interaction = 'TRUE'
clrs = 'homolateral'
...
for iprcnt, prcnt in enumerate([20,50,80]):
    ...
    ax.text(np.mean(xlim)+((xlim[1]-xlim[0])*0.75/4)*(iprcnt-1), 1.25*np.pi, f"{prcnt}th", ha = 'center', color = FigConfig.colour_config[clrs][iprcnt*2])
         
# ax.text(np.mean(xlim), 1.37*np.pi, "weight percentile", color = FigConfig.colour_config[clrs][0], ha = 'center')
pct_text = "weight percentile"
...
figtitle = f"{yyyymmdd}_homolateral_COMBINED_weight_hh.svg"
