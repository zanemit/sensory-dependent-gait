...
def plot_figS2J(yyyymmdd1, yyyymmdd2, param, refLimb="lH1", appdx=""):
    yyyymmdd = [yyyymmdd1, yyyymmdd2]
    ...
    mouselist = Config.passiveOpto_config['mice']+Config.passiveOpto_config['mice_pilot']
    ...
    # upper_lim = 0.375; lower_lim = 0.05
    ...
    for s, inj_side in enumerate(['right', 'left']):
        ...
        for il, limb in enumerate(limbs):
            ...
            ax.scatter(np.repeat(il+(2*s)+scatter_shift, 
                       frac_arr.shape[0]), 
                       frac_arr[:,il], 
                       color =  clr, alpha = 0.8, s = 4, zorder = 3)
    ...
    for inj_id, (hline_X1, hline_X2, hline_Y, text_Y) in enumerate(zip(
            [0.1, 1], [2.1,3], [0.15,0.905], [0.05,0.90]
            )):
        ...
        ax.text(inj_id+1, text_Y, p_text, ha='center', color='grey')
    ...
    ax.text(0.5,1,"RIGHT", ha='center', color='grey', fontsize=5)
    ax.text(2.5,1,"LEFT", ha='center', color='grey', fontsize=5)
    ax.set_ylabel(f"Fraction of {side_str}-leading\nout-of-phase strides")
    ax.set_title("Passive treadmill")
    
    fig.tight_layout()
    
    filename = "passiveOpto_lateralisation.svg"
...
if __name__=="__main__":
    ...
    plot_figS2J(yyyymmdd1, yyyymmdd2, param)