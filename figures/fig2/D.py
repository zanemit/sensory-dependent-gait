...
fig, ax = plt.subplots(1, plot_num,  
                       ...
                       figsize = (1*plot_num,1.63), subplot_kw = dict(projection = 'polar'))
...
for i, gkey in enumerate(keys[[0,group_num-1]]):    
    ...
    ax[i].plot(kde_bins_points, polar_points, color = FigConfig.colour_config[clr_str][2],linestyle = 'solid', linewidth = 1)

ax[0].text(2.6,y_maxlim + (y_maxlim/0.9),'homolateral',size=6, weight = 'bold')
plt.text(0.03, 0.1, 'Probability\ndensity', ha = 'center', color = 'lightgrey', size = 6, transform = plt.gcf().transFigure)
plt.text(0.03, 0.87, 'Incline\nangle', ha = 'center', color = 'black', size = 6, transform = plt.gcf().transFigure)
plt.tight_layout(h_pad = 6) 
fig.savefig(Path(FigConfig.paths['savefig_folder']) / f"MS3_{yyyymmdd}_allPhaseHistogramsAVERAGED_POLAR_{param}{group_num}_{dataToPlot}_{limbRef}{appdx}.svg")
