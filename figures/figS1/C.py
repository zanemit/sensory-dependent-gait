...
ax.plot(x_pred, y_predQDR, linewidth=2, color=FigConfig.colour_config[limb_clr][0]) 
p_text = "head height" + ' ' + ('*' * (modQDR['Pr(>|t|)'][1] < FigConfig.p_thresholds).sum())
if (modQDR['Pr(>|t|)'][1] < FigConfig.p_thresholds).sum() == 0:
    p_text += "n.s."
ax.text(0.6,1.46, p_text, ha = 'center', color = FigConfig.colour_config[limb_clr][0])
        
ax.set_ylabel("Hindlimb weight fraction")
