...
if (modLIN['Pr(>|t|)'][1] < FigConfig.p_thresholds).sum() == 0:
    p_text += "n.s."
ax.text(0.6,0.9, p_text, ha = 'center', color = FigConfig.colour_config[clr][2])
...
fig.savefig(os.path.join(FigConfig.paths['savefig_folder'], f'MS2_{yyyymmdd}_CoMxCoMy_{param}.svg'))
