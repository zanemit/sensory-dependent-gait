...
fig, axes = plt.subplots(2,1, figsize = (1.4,2.3), sharex = True)

for yyyymmdd, ax, title, param, level_dict, ylims in zip(['2021-10-26', '2022-04-04'], 
                                                  [axes[0], axes[1]],
                                                  ['Head height trials','Slope trials'], 
                                                  ['headHW', 'levels'],
    ...
    scalebars.add_scalebar(ax, matchx=True, matchy=False, hidex=True, hidey=True, 
                            loc = 'lower left', pad = 1.9, labelx = "3 cm")

    handles, labels = plt.gca().get_legend_handles_labels()
    by_label = dict(zip(labels,handles))

    if param == 'levels':
        bbox_coords = (0.95,0.3,0.45,0.1)
        lgd_tlt = 'Incline'
    elif param == 'headHW':
        bbox_coords = (0.95,0.75,0.45,0.1)
        lgd_tlt = 'Head height'
...
fig.savefig(os.path.join(FigConfig.paths['savefig_folder'], f'MS2_{yyyymmdd}_sillhouette_headLVL_incline.svg'), 
