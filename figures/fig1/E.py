...
def plot_fig1E(yyyymmdd, param):
    ...
    print(f"Average slope across mice: {np.mean(slopes):.2f} ± {np.std(slopes)/np.sqrt(len(slopes)):.2f}")
    ...
    savepath = os.path.join(FigConfig.paths['savefig_folder'], f'forceplate_totalWeight_{param}_{variable_str}.svg')
