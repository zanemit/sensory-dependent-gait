...
mouselist = Config.passiveOpto_config['egr3_ctrl_mice']
...
data_split = np.asarray([147,155,168,176])
...
for im, m in enumerate(mouselist):
    ...
    # print(f"{m}: {np.nanmin(df_sub[param]):.0f}, {np.nanmax(df_sub[param]):.0f}")
...
fig.savefig(Path(FigConfig.paths['savefig_folder']) / f"thesis_EGR3_{yyyymmdd}_allPhaseHistogramsAVERAGED_POLAR_{param}{group_num}_ALL_{limbRef}_combinedTrials.svg",
            dpi=300,
            transparent=True)
