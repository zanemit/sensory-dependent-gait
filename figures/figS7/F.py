...
def plot_figS7F():
    ...
    for iprcnt, (prcnt, speed, lnst) in enumerate(zip(prcnts,
        ...
        for k, (lo, hi) in enumerate(zip([-np.pi, 0, np.pi] , [np.pi, 2*np.pi, 3*np.pi])):
            ...
            if k == 2:
                ...
                ax.hlines(ylim[1]-0.7, -21+20*iprcnt, -10+20*iprcnt, color = c, ls = lnst, lw = 1)
                ax.text(xlim[0] + (0.23 * (xlim[1]-xlim[0])) + 20*iprcnt,
    ...
    cont_coef_str = "pred1"
    # ax.text(xlim[0] + (0.25 * (xlim[1]-xlim[0])),
    #         ylim[1] - (0.15* (ylim[1]-ylim[0])),
    #         f"DF ratio: {stat_dict[cont_coef_str]}",
    #         fontsize=5)
    ax.text(xlim[0] + (0.13 * (xlim[1]-xlim[0])),
            ylim[1] - (0.03* (ylim[1]-ylim[0])),
            f"slope x DF ratio: {stat_dict['pred1:pred3']}",
            fontsize=5)
